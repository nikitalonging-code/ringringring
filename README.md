...
Заявки на вывод приходят каждому администратору в личные сообщения бота с
кнопками «Принять» и «Отклонить». При принятии пользователь получает
уведомление о выполненном выводе. При отклонении бот запрашивает причину и
предлагает отдельную кнопку для отказа без причины; в обоих случаях сумма
автоматически возвращается на баланс пользователя.
...
### Фрибеты
Администратор пишет боту в личные сообщения `/freebet`. Бот по шагам спрашивает количество активаций, размер фрибета в Stars и wager, после чего создаёт уникальную Mini App-ссылку `fb_<UUID>`. При первом открытии ссылки пользователь получает Stars на баланс, а если задан wager — соответствующее требование отыгрыша. Повторная активация одним пользователем невозможна, а лимит ссылок контролируется в PostgreSQL.

Для прямого открытия ссылки фрибета у бота должен быть настроен Main Mini App или `TELEGRAM_MINI_APP_SHORT_NAME`.

### Создание заданий
Создание задания теперь доступно не только администратору. Администраторы создают задания бесплатно. Для обычных пользователей сохраняется формула текущей версии проекта: `reward × activations × TASK_PRICE_MULTIPLIER`, по умолчанию `1.5`. Бот должен быть администратором указанного публичного канала.

### Приветствие бота
Поведение `/start` и `/help` настраивается через Render: `WELCOME_TEXT`, `WELCOME_IMAGE_URL`, `WELCOME_APP_URL`, `WELCOME_CHANNEL_URL`, `WELCOME_SUPPORT_URL`. Если ссылки/изображение не указаны, используются безопасные значения по умолчанию.
