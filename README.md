...
## Новые настройки и функции

- Админ-панель → «Тех. работы»: runtime-переключатель закрытия Mini App. Администраторы продолжают иметь доступ, обычные пользователи видят экран техработ.
- В админке при выдаче Stars можно указать `Вагер x`; при выдаче суммы `N` и вагере `xK` к аккаунту добавляется wager `N×K`.
- Промокоды поддерживают `Мин. депозит ⭐`: активация разрешается после накопленного реального депозита не меньше указанной суммы.
- GRAM/TON-пополнение больше не требует обязательного `TONAPI_KEY` и пытается читать комментарий платежа из разных форматов ответа TonAPI; ключ всё равно рекомендуется для стабильных лимитов.
- После успешного пополнения Stars или GRAM/TON бот отправляет пользователю уведомление с суммой и кнопкой «🎮 ИГРАТЬ».
- Создание задания открывается отдельной кнопкой; каждая попытка создания задания уведомляет администраторов данными пользователя, канала, награды, активаций и стоимости.
- Бот поддержки использует отдельный webhook на том же Render-сервисе. `/start` предлагает кнопку «СОЗДАТЬ ЗАЯВКУ», заявки приходят администраторам с кнопками «Ответить» и «Закрыть».


### Администрирование, оплаты и поддержка

- В админ-панели есть вкладка `Тех. работы`: она сохраняется в PostgreSQL и закрывает Mini App для обычных пользователей, оставляя доступ администраторам.
- Выдача баланса поддерживает отдельный множитель вагера; сумма нового требования равна `выданные Stars × вагер`.
- Промокоды поддерживают обязательный накопленный депозит пользователя (`required_deposit`) и, отдельно, вагер.
- После успешного платежа Stars пользователь получает личное сообщение от основного бота с суммой пополнения и кнопкой `🎮 ИГРАТЬ`.
- Для GRAM/TON автозачисления обязательны `TON_TOPUP_WALLET_ADDRESS` и `TON_PER_STAR`. `TONAPI_KEY` рекомендуем, но он не обязателен. Автосопоставление выполняется по уникальному memo `RING:<UUID>`, которое генерирует приложение.
- Бот поддержки работает на том же Render Web Service через `/api/support/webhook`. Каждый оператор поддержки должен один раз открыть бота и нажать `Start`, иначе Telegram не разрешит боту отправлять ему личные сообщения.
- `/freebet` не публикуется в списке slash-команд основного бота, но остаётся доступной администраторам при ручном вводе.


## Админ: статистика пользователя и рассылки

В Telegram-бот добавлены админские команды:
- отправь боту в ЛС `@username` пользователя → появятся кнопки `Сегодня`, `7 дней`, `30 дней`, `Всё время`;
- статистика показывает баланс, общий депозит, ставки, игры/winrate, источники начислений, выводы и последние операции;
- `/broadcast` → выбрать всех пользователей или пользователей с депозитом от заданной суммы → отправить боту сообщение для рассылки;
